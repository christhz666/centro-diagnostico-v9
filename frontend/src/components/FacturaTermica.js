    ...
    <div style={{ padding: '10px', maxWidth: '80mm', margin: '0 auto', fontFamily: 'Arial, sans-serif', fontSize: '12px' }}>
      ...
      <div className="factura-termica">
        ...
        <div style={{ marginTop: '15px', textAlign: 'center', borderTop: '2px solid ' + colores.azulCielo, paddingTop: '10px' }}>
          <p style={{ margin: '0 0 5px 0', fontSize: '10px', fontWeight: 'bold', color: colores.azulOscuro }}>CODIGO DE MUESTRA</p>
          <Barcode value={factura?.codigoQR || codigoPaciente} width={1.5} height={40} fontSize={12} margin={5} />
        </div>

        {/* Credenciales de acceso a resultados */}
        {(factura?.pacienteUsername || factura?.codigoQR) && (
          <div style={{ marginTop: '10px', border: '2px dashed ' + colores.azulCielo, borderRadius: 8, padding: '10px', textAlign: 'center', fontSize: '10px' }}>
            <p style={{ margin: '0 0 5px 0', fontWeight: 'bold', color: colores.azulOscuro, fontSize: '11px' }}>🔐 ACCEDA A SUS RESULTADOS EN LÍNEA</p>
            <p style={{ margin: '2px 0', color: '#333' }}>🌐 {window.location.origin}/resultados</p>
            {factura?.pacienteUsername && (
              <>
                ...
                <p style={{ margin: '2px 0', color: '#555' }}>
                  <strong>Contraseña:</strong> {factura.pacientePassword}
                </p>
              </>
            )}
            {factura?.codigoQR && (
              <p style={{ margin: '4px 0 0 0', fontSize: '9px', color: '#888' }}>
                O escanee el código QR de esta factura
              </p>
            )}
          </div>
        )}
